...
const gqlDir = process.cwd() + "/src/poe/graphql";
const queries = {
    ...
    loginWithVerificationCodeMutation: readFileSync(gqlDir + "/LoginWithVerificationCodeMutation.graphql", "utf8"),
    ...
    async signInOrUp(phoneNumber, email, verifyCode) {
        ...
        try {
            const { data: { loginWithVerificationCode: { status: loginStatus }, }, } = await this.makeRequest({
                query: `${queries.loginWithVerificationCodeMutation}`,
                variables: {
